...
class BertSelfAttentionMaskedStd(torch.nn.Module):
    ...
    def kq_to_scores(
        ...
        if attention_mask is not None:
            ...
            nan_scores = torch.where((1-attention_mask).bool(), attention_scores, torch.nan)
            attention_mask = attention_mask.repeat(1,attention_scores.shape[1],attention_scores.shape[2],1)
            std = nanstd(nan_scores, dim=-1)
            std = torch.stack([std for _ in range(attention_scores.shape[-1])], dim=-1)
            std = torch.where(attention_mask.bool(), std, 1)
